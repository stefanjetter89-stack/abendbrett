...
/* --- Speicher: window.storage im Artefakt, localStorage im eigenen Build --- */
const store = {
  async get(key) {
    try {
...
export default function Abendbrett() {
  ...
  useEffect(() => {
    let live = true;
    (async () => {
      const c = await store.get("abendbrett:custom");
      const h = await store.get("abendbrett:hidden");
      if (!live) return;
  ...
  useEffect(() => { if (loaded) store.set("abendbrett:custom", custom); }, [custom, loaded]);
  useEffect(() => { if (loaded) store.set("abendbrett:hidden", hidden); }, [hidden, loaded]);
